...
<template>
        <div class="flex items-center justify-center p-10">
                <div class="w-full max-w-md p-8 bg-white shadow-lg rounded-lg">
                        <h2 class="text-2xl font-bold text-center text-secondary-light">Admin registration</h2>
                        <p class="text-center text-gray-600">Create an admin account</p>
                        <form class="mt-6 justify-center items-center space-y-10">
                                <div class="flex flex-col w-full h-fit items-center">
                                        ...
                                        <div class="flex flex-col space-y-5 w-full h-fit items-center">
                                                <FormsInput class="border-secondary" v-model="user.email" placeholder="example@mail.pt" input-type="email"
                                                :error-message="storeError.fieldMessage('email')" label="Email:" :as-errors="isEmpty('email')" />
                                                <FormsInput v-model="user.name" placeholder="John Doe"
                                                :error-message="storeError.fieldMessage('name')" label="Name:" :as-errors="isEmpty('name')" />
                                ...
                                <button @click.prevent="submit" type="submit"
                                        class="w-full px-4 py-2 mt-6 text-white bg-secondary-light hover:bg-secondary-dark rounded-md focus:outline-none focus:ring-2 focus:ring-tertiary-light">Register</button>
                        </form>
