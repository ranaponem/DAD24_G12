...
<template>
        <div class="flex flex-col items-center justify-center flex-grow">
                ...
                <div v-show="storeAuth.user && selectedGameType === 'S'" class="flex items-center justify-center space-x-4 mt-2 mb-6">
                        <span
                        ...
                        ]" :disabled="isSpinning" class="cursor-pointer disabled:opacity-50">
                                <svg class="object-cover w-full h-full" fill="currentColor" viewBox="0 0 24 24"
                ...
                <div v-show="selectedGameType === 'S'" class="flex flex-row items-center justify-between w-fit">
                        <div
                                class="bg-white p-4 mb-4 mr-4 flex items-center space-x-2 rounded-3xl border-2 border-primary justify-center">
                                <label for="boardType" class="text-lg font-bold text-gray-700 mr-2">
                                        Board Type:</label>
                                <select id="boardType" v-model="selectedBoard"
                        ...
                        <div
                                class="bg-white p-4 mb-4 flex items-center space-x-2 rounded-3xl border-2 border-primary justify-center">
                                <button :class="['px-6 py-2 rounded-full text-lg', selectedType === 'time' ? 'bg-primary text-white' : 'bg-white hover:bg-primary-light text-gray-800']"
                                        @click="selectedType = 'time'">
                                        TIME
                                </button>
                                <button :class="['px-6 py-2 rounded-full text-lg', selectedType === 'turns' ? 'bg-primary text-white' : 'bg-white hover:bg-primary-light text-gray-800']"
                ...
                <div
                        ...
                        <button :class="['px-6 py-2 rounded-full text-lg', selectedGameType === 'S' ? 'bg-primary text-white' : 'bg-white hover:bg-primary-light text-gray-800']"
                                @click="selectedGameType = 'S'">
                                SinglePlayer
                        </button>
                        <button :class="['px-6 py-2 rounded-full text-lg', selectedGameType === 'M' ? 'bg-primary text-white' : 'bg-white hover:bg-primary-light text-gray-800']"
                ...
                <div class="flex items-center justify-center w-full mb-8">
                        <table v-if="selectedGameType === 'M'"
                                ...
                                <thead>
                                        <tr class="bg-white">
                                                ...
                                                <th class="bg-primary px-4 py-2 text-xl text-stone-900">User Nickname
                                                </th>
                                                <th class="bg-primary px-4 py-2 text-xl text-stone-900">Total Games Won
                                                </th>
                                        </tr>
                                </thead>
                                <tbody>
                                        <tr v-for="(game, index) in allGames.data" :key="index"
                                                :class="index % 2 === 0 ? 'bg-white' : 'bg-stone-200'"
                                                class="transition-colors duration-200 text-black">
                                                <td class="px-4 py-2">{{ index + 1 }}</td>
                                                <td class="px-4 py-2">{{ game.user ? game.user.nickname : '-----' }}
                                                </td>
                        ...
                        <table v-else class="table-auto border-collapse w-3/4 text-center rounded-lg overflow-hidden">
                                <thead>
                                        <tr class="bg-white">
                                ...
                                <tbody>
                                        <tr v-for="(game, index) in allGames" :key="game.id"
                                                :class="index % 2 === 0 ? 'bg-white' : 'bg-stone-200'"
                                                class="transition-colors duration-200 text-black">
                                                <td class="px-4 py-2">
                                                        {{ game.won_by ? game.won_by.nickname : (game.created_by ?
                                                ...
                                                <td class="px-4 py-2">{{ game.total_time }}sec</td>
